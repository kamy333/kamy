...
// non post from GET
    ...
    //    $modele_id=(int) $modele_id;
        ...
        $query.="validated_chauffeur, ";
        $query.="validated_mgr, ";
        $query.="validated_final, ";
        $query.="course_date, ";
    //    $query.="modele_id, ";
        $query.="client_id, ";
        $query.="pseudo, ";
  ...
  //      var_dump($last_id);
        ...
        if ($result) {
            // Success
            $_SESSION["message"] = "Course ajoutée pour {$pseudo} le {$date_fr_short} à {$heure}  avec un nouveau ID " . $last_id. " Vous devez la valider ";
            $_SESSION["OK"] = true;
        //    unset($_POST);
        } else {
            // Failure
...
<!--<h4 class="text-center"><mark>xxxxx</mark></h4>-->
...
<?php if(isset($_GET["program_id"])) {
 ...
 list ($date_fr,$date_fr_short,$date_fr_long,$date_fr_hr,$date_fr_short_hr,$date_fr_long_hr,$date_fr_full_hr)= date_fr($program["course_date"]);
    ...
    // GET request
    $fiedset_msg= " Copie du Client " . $program["pseudo"];
...
$missing=validate_presences_non_post($required_fields,$program,true);
    foreach ($program as $key => $val){
        $msg_key= "msg_presence_warning".$key;
        $text="<strong>&nbsp;&nbsp;  ".ucfirst($key)."</strong> est à compléter";
        $$msg_key= array_key_exists ($key, $missing)? get_warning_error_p($text,true)  : "" ;
    //    $$msg_key= array_key_exists ($key, $missing)? get_warning_error($missing[$key],true)  : "" ;
        ...
        <div class ="background_light_blue">
            ...
            <form name="form1" id="" class="form-horizontal" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
                ...
                <fieldset id="login" title="Course">
                    <legend class="text-center" style="color: #0000ff">Nouvelle course <?php echo htmlentities($fiedset_msg, ENT_COMPAT, 'utf-8'); ?></legend>
