...
//function validate_presences_non_post($required_fields,$record) {
//// not coming from post but by query db returning record set
//    global $errors;
...
if (empty($errors)) {
    ...
    if ($result && mysqli_affected_rows($connection) == 1) {
        // Success
        $_SESSION["message"] = "Validation Mgr successfully statut change.";
        $_SESSION["OK"] = true;
        unset($_GET);
        ...
        $_SESSION["message"] = "Validation Mgr update failed because no change was made compare to what existed in db already.";
        //  redirect_to("manage_program.php");
        redirect_to($url);
        ...
        // Failure
        $_SESSION["message"] = "Validation Mgr statut change failed.";
        //   redirect_to("manage_program.php");
        redirect_to($url);
